...
class DataGenerator(object):
    def __init__(self, step=10, window=20, max_freq=8000, desc_file=None):
        ...
        """
        self.feat_dim = calc_feat_dim(window, max_freq) #8000*20*0.001+1 = 161
        self.feats_mean = np.zeros((self.feat_dim,)) #zero padding for length audio
    ...
    def prepare_minibatch(self, audio_paths, texts):
        ...
        return {
            'x': x[..., np.newaxis],  # (0-padded features of shape(mb_size,timesteps,feat_dim)
    ...
    def iterate_train(self, minibatch_size=36, sort_by_duration=False,
                      shuffle=True):
    ...
    def iterate_test(self, minibatch_size=36):
        return self.iterate(self.test_audio_paths, self.test_texts,
                            minibatch_size)

    def iterate_validation(self, minibatch_size=36):
